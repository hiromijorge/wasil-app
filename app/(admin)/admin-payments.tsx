...
import { useAllMerchantPayoutRequests, useProcessMerchantPayout } from "../../src/hooks/useMerchantPayoutRequests";
import { useAllDriverPayoutRequests, useProcessDriverPayout } from "../../src/hooks/useDriverPayoutRequests";
import { useAllPartnerPayoutRequests, useProcessPartnerPayout } from "../../src/hooks/usePartnerPayouts";
import { useCashCollections } from "../../src/hooks/useCashCollections";
import { formatSAR } from "../../src/lib/format";
import { useTranslation } from "../../src/lib/i18n";
import { palette, fonts, spacing, radii, shadows } from "../../src/lib/theme";
import { Button } from "../../src/components/Button";

type Tab = "receipts" | "merchant" | "driver" | "parcels" | "withdrawals" | "cash";
...
export default function AdminPaymentsScreen() {
  ...
  const merchantRequests = useAllMerchantPayoutRequests();
  const driverRequests = useAllDriverPayoutRequests();
  const partnerRequests = useAllPartnerPayoutRequests();
  const cashCollections = useCashCollections();
  const processMerchantRequest = useProcessMerchantPayout();
  const processDriverRequest = useProcessDriverPayout();
  const processPartnerRequest = useProcessPartnerPayout();
  ...
  const isLoading =
    ...
    (activeTab === "withdrawals" && (merchantRequests.isLoading || driverRequests.isLoading || partnerRequests.isLoading)) ||
    (activeTab === "cash" && cashCollections.isLoading);
      ...
      {!isAdmin && !authLoading ? (
        ...
        >
          ...
          {activeTab === "withdrawals" && (
            <View>
              <Text style={styles.title}>{t("withdrawalRequests")}</Text>
              <Text style={styles.subtitle}>{t("withdrawalRequestsSubtitle")}</Text>
            </View>
          )}

          {activeTab === "cash" && (
            <View>
              <Text style={styles.title}>{t("cashCollections")}</Text>
              <Text style={styles.subtitle}>{t("cashCollectionsSubtitle")}</Text>
            </View>
          )}
          ...
          {activeTab === "cash" && !cashCollections.isLoading && (
            <View style={{ gap: spacing.md }}>
              {cashCollections.data.length === 0 ? (
                <View style={[styles.card, shadows.card]}>
                  <EmptyState message={t("noCashCollections")} />
                </View>
              ) : (
                cashCollections.data.map((summary) => (
                  <CashCollectionCard
                    key={summary.driver_id}
                    summary={summary}
                    onSettle={async () => {
                      if (!adminId) return;
                      await cashCollections.settle(summary.driver_id, summary.unsettled_balance_sar, adminId);
                    }}
                  />
                ))
              )}
          ...
          {activeTab === "withdrawals" &&
            !merchantRequests.isLoading &&
            !driverRequests.isLoading &&
            !partnerRequests.isLoading && (
              <View style={{ gap: spacing.md }}>
                <WithdrawalSection
                  title={t("merchantWithdrawals")}
                  requests={(merchantRequests.data ?? []).map((r) => ({
                    ...r,
                    recipientName: r.merchant?.full_name ?? r.merchant?.email ?? t("unknownMerchant"),
                    payoutDetails: r.merchant?.payout_details ?? null,
                  }))}
                  onProcess={async (id, status) => {
                    if (!adminId) return;
                    await processMerchantRequest.mutateAsync({ requestId: id, status, processedBy: adminId });
                  }}
                  processing={processMerchantRequest.isPending}
                />
                <WithdrawalSection
                  title={t("driverWithdrawals")}
                  requests={(driverRequests.data ?? []).map((r) => ({
                    ...r,
                    recipientName: r.driver?.full_name ?? r.driver?.email ?? t("unknownDriver"),
                    payoutDetails: r.driver?.payout_details ?? null,
                  }))}
                  onProcess={async (id, status) => {
                    if (!adminId) return;
                    await processDriverRequest.mutateAsync({ requestId: id, status, processedBy: adminId });
                  }}
                  processing={processDriverRequest.isPending}
                />
                <WithdrawalSection
                  title={t("partnerWithdrawals")}
                  requests={(partnerRequests.data ?? []).map((r) => ({
                    ...r,
                    recipientName: r.partner?.full_name ?? r.partner?.email ?? t("role_partner"),
                    payoutDetails: r.partner?.payout_details ?? null,
                  }))}
                  onProcess={async (id, status) => {
                    if (!adminId) return;
                    await processPartnerRequest.mutateAsync({ requestId: id, status, processedBy: adminId });
                  }}
                  processing={processPartnerRequest.isPending}
                />
              </View>
            )}
        </ScrollView>
    ...
    <View style={styles.receiptCard}>
      ...
      <View style={styles.receiptBody}>
        ...
        <View style={styles.receiptAmountRow}>
          ...
          <View style={[styles.methodBadge, order.payment_method === "cash" && styles.methodBadgeCash]}>
            <Text style={[styles.methodBadgeText, order.payment_method === "cash" && styles.methodBadgeTextCash]}>
              {order.payment_method === "cash" ? t("cashOnDelivery") : t("bankTransfer")}
            </Text>
          </View>
        </View>
    ...
    <View style={styles.receiptCard}>
      ...
      <View style={styles.receiptBody}>
        ...
        <View style={styles.receiptAmountRow}>
          ...
          <View style={[styles.methodBadge, parcel.payment_method === "cash" && styles.methodBadgeCash]}>
            <Text style={[styles.methodBadgeText, parcel.payment_method === "cash" && styles.methodBadgeTextCash]}>
              {parcel.payment_method === "cash" ? t("cashOnDelivery") : t("bankTransfer")}
            </Text>
          </View>
        </View>
        <Text style={styles.receiptDate}>
...
function WithdrawalSection({
  ...
  onProcess: (id: string, status: "approved" | "rejected" | "paid") => Promise<void>;
  processing: boolean;
}) {
  const { t } = useTranslation();
  ...
  return (
    <View style={[styles.card, shadows.card]}>
      ...
      {requests.length === 0 ? (
        <EmptyState message={t("noWithdrawalRequests")} />
      ) : (
        requests.map((r) => (
...
function WithdrawalRequestCard({
  ...
  const { t } = useTranslation();
  const details = request.payoutDetails as Record<string, string> | null;
  return (
    <View style={styles.payoutCard}>
      <View style={styles.payoutBody}>
        <Text style={styles.payoutName}>{request.recipientName}</Text>
        <Text style={styles.payoutAmount}>{formatSAR(Number(request.amount_sar))}</Text>
        <Text style={styles.payoutDate}>{new Date(request.requested_at).toLocaleDateString()}</Text>
        {details && (
          <View style={{ marginTop: spacing.xs }}>
            {details.bankName ? <Text style={styles.payoutMeta}>{t("bankName")}: {details.bankName}</Text> : null}
            {details.accountHolder ? <Text style={styles.payoutMeta}>{t("accountHolder")}: {details.accountHolder}</Text> : null}
            {details.accountNumber ? <Text style={styles.payoutMeta}>{t("accountNumber")}: {details.accountNumber}</Text> : null}
            {details.iban ? <Text style={styles.payoutMeta}>{t("iban")}: {details.iban}</Text> : null}
          </View>
        )}
      </View>
        ...
        {request.status === "pending" ? (
          <>
            <Button
              ...
              size="sm"
              variant="outline"
              onPress={() => onProcess(request.id, "approved")}
              loading={processing}
            />
            <Button
              title={t("reject")}
              size="sm"
              variant="outline"
              onPress={() => onProcess(request.id, "rejected")}
              loading={processing}
            />
          </>
          ...
          <Button
            title={t("markPaid")}
            size="sm"
            onPress={() => onProcess(request.id, "paid")}
            loading={processing}
          />
        ) : (
          <Text
...
function CashCollectionCard({
  ...
  summary: import("../../src/hooks/useCashCollections").DriverCashSummary;
  onSettle: () => void;
}) {
  const { t } = useTranslation();
  const hasUnsettled = summary.unsettled_balance_sar > 0;
  return (
    <View style={[styles.card, shadows.card]}>
      <View style={styles.cashHeader}>
        <View>
          <Text style={styles.payoutName}>{summary.driver_name ?? t("unknownDriver")}</Text>
          <Text style={styles.payoutMeta}>
            {summary.orders.length} {t("ordersTab").toLowerCase()} · {summary.parcels.length} {t("sendsTab").toLowerCase()}
          </Text>
        </View>
        {hasUnsettled && (
          <Button title={t("settleDriver")} size="sm" onPress={onSettle} />
        )}
      </View>

      <View style={styles.cashRow}>
        <View style={styles.cashCell}>
          <Text style={styles.cashLabel}>{t("collected")}</Text>
          <Text style={styles.cashValue}>{formatSAR(summary.total_collected_sar)}</Text>
        </View>
        <View style={styles.cashCell}>
          <Text style={styles.cashLabel}>{t("driverFee")}</Text>
          <Text style={styles.cashValue}>{formatSAR(summary.driver_fee_sar)}</Text>
        </View>
        <View style={styles.cashCell}>
          <Text style={styles.cashLabel}>{t("remittanceDue")}</Text>
          <Text style={styles.cashValue}>{formatSAR(summary.remittance_due_sar)}</Text>
        </View>
      </View>

      <View style={styles.cashBalanceRow}>
        <Text style={styles.cashBalanceLabel}>{t("unsettledBalance")}</Text>
        <Text style={[styles.cashBalanceValue, !hasUnsettled && { color: palette.success }]}>
          {formatSAR(summary.unsettled_balance_sar)}
        </Text>
      </View>
    </View>
  );
